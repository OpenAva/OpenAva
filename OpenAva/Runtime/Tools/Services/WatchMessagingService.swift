...
final class WatchMessagingService: NSObject, @preconcurrency WatchMessagingServicing {
    private nonisolated static let logger = Logger(subsystem: "ai.openava", category: "watch.messaging")
    ...
    func sendNotification(
        ...
        if snapshot.reachable {
            do {
                try await sendReachableMessage(payload, with: session)
    ...
    private nonisolated static func parseQuickReplyPayload(
