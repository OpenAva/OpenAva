...
extension ConversationSession {
    ...
    func executeInferenceStep(
        ...
        // Clear stale transport errors so an old failure does not leak into this round.
        ...
        if message.textContent.isEmpty, (message.reasoningContent ?? "").isEmpty, pendingToolCalls.isEmpty {
            ...
            // Prefer provider-side error details (for example HTTP 4xx `error.message`) when available.
        ...
        // Append tool calls to the original assistant message instead of creating separate messages
        struct ToolCallEntry {
            let request: ToolRequest
            let tool: ToolExecutor
            let toolCallPartIndex: Int // Index of the toolCall part in the message.parts array
        }
        ...
        for request in pendingToolCalls {
            ...
            // Append tool call directly to the original message
            let partIndex = message.parts.count
            ...
            toolCallEntries.append(ToolCallEntry(request: request, tool: tool, toolCallPartIndex: partIndex))
        }
        await requestUpdate(view: messageListView)
        ...
        for (index, entry) in toolCallEntries.enumerated() {
            ...
            // Update tool call state in the original message
            if entry.toolCallPartIndex < message.parts.count {
                if case var .toolCall(tc) = message.parts[entry.toolCallPartIndex] {
                    tc.state = response.isError ? .failed : .succeeded
                    message.parts[entry.toolCallPartIndex] = .toolCall(tc)
                }
            }
            // Append tool result to the original message
            message.parts.append(
                .toolResult(.init(toolCallID: entry.request.id, result: response.text, isCollapsed: true))
            )
            await requestUpdate(view: messageListView)
