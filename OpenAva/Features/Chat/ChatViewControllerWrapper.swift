...
struct ChatViewControllerWrapper: UIViewControllerRepresentable {
    ...
    private var resolvedSessionTitle: String {
        ...
        case .agent:
            let title = activeAgentName.trimmingCharacters(in: .whitespacesAndNewlines)
            return title.isEmpty ? L10n.tr("chat.activeAgent.fallbackName") : title
        }
    }
    ...
    private var headerTeamMemberCount: Int? {
        switch activeContext {
        case .allAgentsTeam:
            return agents.count
        case let .team(teamID):
    ...
    private var headerState: ChatViewController.HeaderState {
        .init(
    ...
    private var fallbackConversationModels: ConversationSession.Models {
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(
            ...
            activeAgentName: resolvedSessionTitle,
    ...
    private func resolveSessionContext(agentCount: Int) -> (
        ...
        case .allAgentsTeam, .team:
            ...
            providedSession = nil
            models = fallbackConversationModels
        ...
        if isTeamSession {
            sessionConfiguration.storage.setTitle(resolvedSessionTitle, for: sessionID)
        }
    ...
    private func applyPromptSubmissionHandler(
        ...
        if let serializedExecutionContext = sessionContext.serializedExecutionContext {
            ...
            chatViewController.promptSubmissionHandler = { session, _, prompt in
                TeamRoomOrchestrator.shared.submitTeamRoomPrompt(
                    roomSession: session,
                    prompt: prompt,
                    context: teamRoomContext,
                    usingExistingReservation: true
                )
            }
        } else {
    ...
    func updateUIViewController(_ uiViewController: UIViewController, context: Context) {
        ...
        context.coordinator.activeAgentName = resolvedSessionTitle
