...
extension ChatRequest {
    static func normalize(_ messages: [Message]) -> [Message] {
        mergeAssistantMessages(messages).compactMap(normalizeMessage)
    }
    ...
    static func normalizeMessage(_ message: Message) -> Message? {
        ...
        case let .assistant(content, toolCalls, reasoning, thinkingBlocks):
            ...
            let normalizedToolCalls = normalizeToolCalls(toolCalls)
            let normalizedReasoning = trimmed(reasoning)
            let hasToolCalls = !(normalizedToolCalls?.isEmpty ?? true)
            let hasThinkingData = normalizedReasoning != nil || !(thinkingBlocks?.isEmpty ?? true)

            // Some interrupted streams may persist an assistant placeholder with
            // no content/tool calls. Backfill empty content to keep the message
            // shape valid and preserve message ordering for follow-up turns.
            if normalizedContent == nil, !hasToolCalls {
                if hasThinkingData {
                    return .assistant(
                        content: .text(""),
                        toolCalls: nil,
        ...
        case let .developer(content, name):
            return .developer(content: normalizeTextContent(content), name: trimmed(name))
        case let .system(content, name):
            return .system(content: normalizeTextContent(content), name: trimmed(name))
        case let .tool(content, toolCallID):
            return .tool(content: normalizeTextContent(content), toolCallID: trimmed(toolCallID) ?? toolCallID)
        case let .user(content, name):
            return .user(content: normalizeUserContent(content), name: trimmed(name))
        }
    }
