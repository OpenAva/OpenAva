...
public extension ChatRequestBody {
    ...
    /// Returns a copy where assistant messages are guaranteed to include at
    /// least `content` or non-empty `tool_calls`, preventing invalid payloads
    /// caused by interrupted streams leaving reasoning-only placeholders.
    func sanitizingOutboundMessages() -> ChatRequestBody {
        var sanitized = ChatRequestBody(
            messages: messages.map { message in
                switch message {
                case let .assistant(content, toolCalls, reasoning, thinkingBlocks):
                    let normalizedToolCalls = (toolCalls?.isEmpty == true) ? nil : toolCalls
