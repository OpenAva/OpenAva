...
final class QuickSettingBar: EditorSectionView {
        ...
        for item in items {
            ...
            case let .command(_, title, icon, command):
                ...
                button.actionBlock = { [weak self] in
                    self?.delegate?.quickSettingBarDidTriggerCommand(command)
                }
                scrollView.addSubview(button)
                buttons.append(button)
            ...
            case let .skill(_, title, icon, prompt, autoSubmit):
                let button = BlockButton(text: title, icon: icon)
                button.actionBlock = { [weak self] in
                    self?.delegate?.quickSettingBarDidTriggerSkill(prompt: prompt, autoSubmit: autoSubmit)
                }
                scrollView.addSubview(button)
                buttons.append(button)
            }
