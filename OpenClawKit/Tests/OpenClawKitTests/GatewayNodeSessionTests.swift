...
private final class FakeGatewayWebSocketTask: WebSocketTasking, @unchecked Sendable {
    ...
    var state: URLSessionTask.State {
        get { lock.withLock { self._state } }
        set { lock.withLock { self._state = newValue } }
    }
    ...
    func send(_ message: URLSessionWebSocketTask.Message) async throws {
        ...
        {
            lock.withLock { self.connectRequestId = id }
        }
    }
    ...
    func receive() async throws -> URLSessionWebSocketTask.Message {
        let phase = lock.withLock { () -> Int in
        ...
        for _ in 0 ..< 50 {
            let id = lock.withLock { self.connectRequestId }
            if let id {
    ...
    func receive(
        ...
        lock.withLock { self.pendingReceiveHandler = completionHandler }
    }
...
private final class FakeGatewayWebSocketSession: WebSocketSessioning, @unchecked Sendable {
    ...
    func snapshotMakeCount() -> Int {
        lock.withLock { self.makeCount }
    }

    func latestTask() -> FakeGatewayWebSocketTask? {
        lock.withLock { self.tasks.last }
    }
...
struct GatewayNodeSessionTests {
    ...
    func invokeWithTimeoutReturnsUnderlyingResponseBeforeTimeout() async {
        ...
        let response = await GatewayNodeSession.invokeWithTimeout(
            ...
            onInvoke: { req in
                #expect(req.id == "1")
                return BridgeInvokeResponse(id: req.id, ok: true, payload: "{}", error: nil)
            }
        )
    ...
    func invokeWithTimeoutReturnsTimeoutError() async {
        ...
        let response = await GatewayNodeSession.invokeWithTimeout(
            ...
            onInvoke: { _ in
                ...
                return BridgeInvokeResponse(id: "abc", ok: true, payload: "{}", error: nil)
            }
        )
    ...
    func invokeWithTimeoutZeroDisablesTimeout() async {
        ...
        let response = await GatewayNodeSession.invokeWithTimeout(
            ...
            onInvoke: { req in
                ...
                return BridgeInvokeResponse(id: req.id, ok: true, payload: nil, error: nil)
            }
        )
    ...
    func emitsSyntheticSeqGapAfterReconnectSnapshot() async throws {
        ...
        try await gateway.connect(
            ...
            onDisconnected: { _ in },
            onInvoke: { req in
                BridgeInvokeResponse(id: req.id, ok: true, payload: nil, error: nil)
            }
        )
