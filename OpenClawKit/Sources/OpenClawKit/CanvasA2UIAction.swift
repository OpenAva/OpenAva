...
public enum OpenClawCanvasA2UIAction: Sendable {
    ...
    public static func formatAgentMessage(_ context: AgentMessageContext) -> String {
        ...
        return [
            "CANVAS_A2UI",
            "action=\(sanitizeTagValue(context.actionName))",
            "session=\(sanitizeTagValue(context.session.key))",
            "surface=\(sanitizeTagValue(context.session.surfaceId))",
            "component=\(sanitizeTagValue(context.component.id))",
            "host=\(sanitizeTagValue(context.component.host))",
            "instance=\(sanitizeTagValue(context.component.instanceId))\(ctxSuffix)",
    ...
    public static func jsDispatchA2UIActionStatus(actionId: String, ok: Bool, error: String?) -> String {
        ...
        (() => {
          const detail = \(json);
          window.dispatchEvent(new CustomEvent('openava:a2ui-action-status', { detail }));
        })();
        """
    }
}
