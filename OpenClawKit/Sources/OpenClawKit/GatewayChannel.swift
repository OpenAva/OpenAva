...
public actor GatewayChannelActor {
    private let logger = Logger(subsystem: "ai.openava", category: "gateway")
    ...
    private func watchdogLoop() async {
        ...
        while shouldReconnect {
            guard await sleepUnlessCancelled(nanoseconds: 30 * 1_000_000_000) else { return } // 30s cadence
            ...
            do {
                ...
                logger.error("gateway watchdog reconnect failed \(wrapped.localizedDescription, privacy: .public)")
            }
        }
    }
    ...
    public func connect() async throws {
        ...
        do {
            ...
            logger.error("gateway ws connect failed \(wrapped.localizedDescription, privacy: .public)")
            throw wrapped
        }
        listen()
        connected = true
    ...
    private func keepaliveLoop() async {
        while shouldReconnect {
            guard await sleepUnlessCancelled(
                nanoseconds: UInt64(keepaliveIntervalSeconds * 1_000_000_000)
            )
            else { return }
            guard shouldReconnect else { return }
    ...
    private func sendConnect() async throws {
        ...
        let options = connectOptions ?? GatewayConnectOptions(
            role: "operator",
            scopes: defaultOperatorConnectScopes,
            caps: [],
            commands: [],
            permissions: [:],
            clientId: "openava-macos",
            clientMode: "ui",
        ...
        let authToken = includeDeviceIdentity ? (storedToken ?? token) : token
        ...
        logger.info("gateway connect auth=\(authSource.rawValue, privacy: .public)")
        let canFallbackToShared = includeDeviceIdentity && storedToken != nil && token != nil
        if let authToken {
    ...
    private func handleReceiveFailure(_ err: Error) async {
        ...
        logger.error("gateway ws receive failed \(wrapped.localizedDescription, privacy: .public)")
        connected = false
        keepaliveTask?.cancel()
        keepaliveTask = nil
    ...
    private func handle(_ msg: URLSessionWebSocketTask.Message) async {
        ...
        guard let frame = try? decoder.decode(GatewayFrame.self, from: data) else {
            logger.error("gateway decode failed")
            return
        }
        switch frame {
        case let .res(res):
    ...
    private func waitForConnectChallenge() async throws -> String {
        guard let task else { throw ConnectChallengeError.timeout }
        return try await AsyncTimeout.withTimeout(
            seconds: connectChallengeTimeoutSeconds,
    ...
    private func watchTicks() async {
        let tolerance = tickIntervalMs * 2
        while connected {
            guard await sleepUnlessCancelled(nanoseconds: UInt64(tolerance * 1_000_000)) else { return }
            guard connected else { return }
            if let last = lastTick {
                let delta = Date().timeIntervalSince(last) * 1000
                if delta > tolerance {
                    logger.error("gateway tick missed; reconnecting")
                    connected = false
                    await failPending(
                        NSError(
                            domain: "Gateway",
    ...
    private func scheduleReconnect() async {
        ...
        guard await sleepUnlessCancelled(nanoseconds: UInt64(delay * 1_000_000_000)) else { return }
        guard shouldReconnect else { return }
        do {
            try await connect()
        } catch {
            let wrapped = wrap(error, context: "gateway reconnect")
            logger.error("gateway reconnect failed \(wrapped.localizedDescription, privacy: .public)")
            await scheduleReconnect()
        }
    }
    ...
    public func request(
        ...
        try await connectOrThrow(context: "gateway connect")
        let effectiveTimeout = timeoutMs ?? defaultRequestTimeoutMs
        let payload = try encodeRequest(method: method, params: params, kind: "request")
    ...
    public func send(method: String, params: [String: AnyCodable]?) async throws {
        try await connectOrThrow(context: "gateway connect")
        let payload = try encodeRequest(method: method, params: params, kind: "send")
        guard let task else {
            throw NSError(
                domain: "Gateway",
                code: 5,
    ...
    private func connectOrThrow(context: String) async throws {
        do {
            ...
            throw wrap(error, context: context)
        }
    }
