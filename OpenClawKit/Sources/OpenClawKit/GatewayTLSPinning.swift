...
public enum GatewayTLSStore {
    private static let keychainService = "ai.openava.tls-pinning"

    // Legacy UserDefaults location used before Keychain migration.
    private static let legacySuiteName = "ai.openava.shared"
    private static let legacyKeyPrefix = "gateway.tls."

    public static func loadFingerprint(stableID: String) -> String? {
        migrateFromUserDefaultsIfNeeded(stableID: stableID)
        let raw = GenericPasswordKeychainStore.loadString(service: keychainService, account: stableID)?
    ...
    public static func saveFingerprint(_ value: String, stableID: String) {
        _ = GenericPasswordKeychainStore.saveString(value, service: keychainService, account: stableID)
    }
    ...
    private static func migrateFromUserDefaultsIfNeeded(stableID: String) {
        guard let defaults = UserDefaults(suiteName: legacySuiteName) else { return }
        let legacyKey = legacyKeyPrefix + stableID
        guard let existing = defaults.string(forKey: legacyKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines),
            !existing.isEmpty
        else { return }
        if GenericPasswordKeychainStore.loadString(service: keychainService, account: stableID) == nil {
            guard GenericPasswordKeychainStore.saveString(existing, service: keychainService, account: stableID) else {
                return
            }
        }
